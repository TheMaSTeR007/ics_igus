...
from sql_queries import products_links_query, categories_links_query, category_page_table_query
...
class Scraper:
    ...
    def scrape(self):
        ...
        categories_data_fetch_query = f'''SELECT * FROM `{categories_data_table_name}` WHERE category_status = 'Pending';'''
        self.cursor.execute(categories_data_fetch_query)
        ...
        for this_category_data in categories_data:
            ...
            category_link_raw = this_category_data[1]
            category_link = category_link_raw
            print('Category Url: ', category_link)
            category_request_url = this_category_data[1]  # category url fetched from Database Table used for Requesting
            print('Category Request Url: ', category_request_url)
            category_response_url = this_category_data[3]  # Final Url of Category received in Response
            print('Category Response Url: ', category_response_url)
            
            # Checking if Category page exists, else sending request and saving it
            category_page_text = self.page_checker(url=category_link, method='GET', directory_path=os.path.join(self.project_files_dir, 'Categories_Pages'), page_hash=category_filename_hash_, table_name=categories_data_table_name, _id_=category_index_id_)
            ...
            if category_id:
                ...
                for page_no in range(1, pages_count + 1):
                    ...
                    category_link_page_no = '/'.join(category_link.split('/')[:-1]) + '/products/' + category_id + f'?sort=3&inch=false&page={page_no}'
                    ...
                    try:
                        self.cursor.execute(category_page_insert_query, args=(category_link_page_no, category_response_url))
                    except Exception as error:
                        print('Category Page Link Error: ', error)



                    # category_page = self.page_checker(url=category_link_page_no, method='GET', directory_path=os.path.join(self.project_files_dir, 'Category_Pages_page_no'), page_hash=)
