...
def page_checker_json_new(url: str, method: str, directory_path: str, cookies: dict = None, headers: dict = None, query_dict: dict = None, params: dict = None):
    # Create a unique hash for the URL and data to use as the filename
    hash_input = url + json.dumps(query_dict, sort_keys=True)  # Combine URL and data for hashing
    page_hash = hashlib.sha256(hash_input.encode('UTF-8')).hexdigest()
    ensure_dir_exists(dir_path=directory_path)  # Ensure the directory exists
    ...
    def scrape(self):
        ...
        for category_data in categories_data:
            ...
            if cat_id == 4:
                ...
                category_link = 'https://www.igus.com' + category_link_
                print('Category Url: ', category_link)
                category_page_text = page_checker(url=category_link, method='GET', directory_path=os.path.join(project_files_dir, 'Categories_Pages'))
                ...
                if category_id:
                    ...
                    #         print('Ignored List: x', ignored_list)
                        ...
                        # category_link = main_page_url + category_link_
                        # print('+' * 50)
                ...
                # As pre-assembled-cable-carriers does not have products on its page, ignoring it manually
                    ...
                    for page_no in range(1, page_count + 1):
                        ...
                        print(category_link)
                        params = {
                            'categoryL1': 'chainflex',
                            'categoryL2': 'bulk-cables',
                            'params': '2',
                        }

                        category_page = page_checker_json_new(url=category_link, method='GET', directory_path=os.path.join(project_files_dir, 'Products_Pages_Json'), params=params)
